...
- **Block ads** — avoids the "commercial break in progress" placeholder by fetching the live playlist through a third-party proxy that Twitch doesn't serve mid-roll ads to (the same technique as the reference SmartTwitchTV app). Off by default. Two proxies are offered (`ontdb`, `kwabang`) — try either, since availability varies by region and over time. Live streams play through the bundled player so it can **automatically fall back to normal (ad-supported) playback** if the proxy is down, so the stream still starts either way. Note: these are public community proxies, not affiliated with this app or Twitch; the token-passing proxy briefly sees the stream's playback token (not your account login). Ads on the very start of a stream (pre-roll) and banner/display ads are not affected.
